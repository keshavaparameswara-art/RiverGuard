...
export const getRegionCoords = (name: string) => {
    if (!name) return null;
    const region = INDIAN_REGIONS.find(r => r.name.toLowerCase() === name.toLowerCase());
    return region ? [region.lat, region.lng] as [number, number] : null;
};
